odoo.define('hotel_management_simsoft.hotel_room_summary', function (require) {
        ...
        load_form: function(data) {
            ...
            this.$el.find(".table_free").bind("click", function(event){
                self.action_manager.do_action({
                        ...
                        res_model: "hotel.reserver",
                        views: [[false, 'form']],
                        target: 'new',
                        context: {"room_id": $(this).attr("data"), 'date': $(this).attr("date") ,'default_nbr_adult':2},
                });
            });

            this.$el.find(".table_Occupee").bind("click", function(event){
                self.action_manager.do_action({
                        type: 'ir.actions.act_window', 
                        view_mode: 'form' ,
                        res_id: "reservid": $(this).attr("data"),
                        res_model: "hotel.arrivee",
                       /* domain: [['reservation_id', '=', 'R16']],*/
                        views: [[false, 'form']],
                        target: 'new',
